      ...
      manip.yoy.S.deer <- function(cap, mort){
        ...
        tmp.index <- which(cap$Frequency > 155 & nchar(cap$Frequency) == 6)
        if(length(tmp.index) > 0){
          cap$Frequency[tmp.index] <- 
            as.numeric(
              paste(substr(cap$Frequency[tmp.index], 1, 3), 
                    substr(cap$Frequency[tmp.index], 4, 6),
                        sep = "."))
        }
        cap$Frequency[cap$Frequency > 155 & nchar(cap$Frequency) != 6] <- NA
        cap.freq <- as.numeric(cap$Frequency)
        
        #  Mother collared, categorical
        tmp.momtag <- as.numeric(miss.fun(cap$Collared.Mother.Frequency, 
                                          rand = F)
                                 )
        cap.momtag <- ifelse(tmp.momtag > 0 & !is.na(tmp.momtag), 1, 0)
        ...
        tmp.index <- which(mort$Frequency > 155 & nchar(mort$Frequency) == 6) 
        if(length(tmp.index) > 0){
          mort$Frequency[] <- 
            as.numeric(
              paste(substr(mort$Frequency[tmp.index], 1, 3), 
                  substr(mort$Frequency[tmp.index], 4, 6),
                  sep = ".")
            )
        }
