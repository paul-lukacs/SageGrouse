#
# RIDFG_surv.R
# Survival analysis for RIDFG package
# Paul M. Lukacs
# Wildlife Biology Program, The University of Montana
#

getAerialSightData <- function( input ){
#
# extracts aerial sightability data from teh AERIAL database
#
	con <- odbcConnectAccess2007( idfgDbAERIAL )
	
	if( input$spp == "Mule Deer" ){
		species = "Mule Deer"
	} else if( input$spp == "Elk" ){
		species = "Elk"
	} else {
		cat( "Model unavailable for ", input$spp, ".\n"  )
		return(NULL)
	}
	
	qryTxt <- paste( "SELECT * FROM qr_ViewAerialSurveyData WHERE [Bio-Year]='", input$bioyear, "' AND Species='", species, 
					 "' AND GMU_Zone_PMU='", input$gmu, "' AND Survey_Type='Sightability-Abundance'", sep="" )
	sight.data <- sqlQuery( con, qryTxt )
	odbcClose(con)
	return(sight.data)
}


getAerialCompData <- function( input ){
#
# extracts aerial sightability data from the AERIAL database
#
	con <- odbcConnectAccess2007( idfgDbAERIAL )
	if( input$spp == "Mule Deer" ){
		species = "Mule Deer"
	...
	qryTxt <- paste( "SELECT * FROM qr_ViewAerialSurveyData WHERE [Bio-Year]='", input$bioyear, "' AND Species='", species, 
					 "' AND GMU_Zone_PMU='", input$gmu, "' AND Survey_Type='Winter Comp'", sep="" )
	sight.data <- sqlQuery( con, qryTxt )
	odbcClose(con)
	return(sight.data)
}
...
#
	pp <- c( -0.254, 1.562, 4.430, -0.888, -2.383,  -0.602, -0.634, -1.368, -0.598, 0.047 )
#	p.int <- -0.254
...
#	if( data$snow > 0 )
	...
	options(warn=-1)
	data$logitp <- p.int+p.act[data$Activity]+p.veg[data$Veg_Type]+p.snw[data$snow]+p.grp*data$Total
...
#
	...
	if( input$spp == "Mule Deer" ){
	...
	} else if( input$spp == "Elk" && input$aircraft == "Hiller 12E" ) { # elk hiller 12e with snow
		npar <- 4
		Sigma <- matrix( NA, npar, npar )
		Sigma[1,1] <- c( 0.00187 )
		...
		for( i in 1:npar ) Sigma[i,]<-Sigma[,i]
	} else if( input$spp == "Elk" && input$aircraft == "Hiller 12E no snow" ) { # elk hiller 12e with snow
		npar <- 3
		Sigma <- matrix( NA, npar, npar )
		Sigma[1,1] <- c( 0.00504 )
		...
		for( i in 1:npar ) Sigma[i,]<-Sigma[,i]
	} else if( input$spp == "Elk" && input$aircraft == "Bell 47G" ) { # elk hiller 12e with snow
		npar <- 3
		Sigma <- matrix( NA, npar, npar )
		Sigma[1,1] <- c( 0.00411 )
...
#
	...
	data$VegClass <- ifelse( data$Perc_Screen_Cover > 27.5 && data$Perc_Screen_Cover < 42.5 , 3, data$VegClass )
	data$VegClass <- ifelse( data$Perc_Screen_Cover > 42.5 && data$Perc_Screen_Cover < 57.5 , 4, data$VegClass )
	data$VegClass <- ifelse( data$Perc_Screen_Cover > 57.5 && data$Perc_Screen_Cover < 72.5 , 5, data$VegClass )
	data$VegClass <- ifelse( data$Perc_Screen_Cover > 72.5 && data$Perc_Screen_Cover < 87.5 , 6, data$VegClass )
	data$VegClass <- ifelse( data$Perc_Screen_Cover > 87.5, 7, data$VegClass )
	...
	options(warn=-1)
	data$logitp <- p.int+p.veg*data$VegClass+p.snw*data$snowCube+p.grp*data$Total
	data$p <- 1/(1+exp(-(data$logitp)))
	for( i in 1:length(data$Total) )
		data$selogitp[i] <- sqrt( t(dm[i,]) %*% sigma(input) %*% dm[i,] )
	data$sep <- (exp(-data$logitp)/(1+exp(-data$logitp))^2)*data$selogitp
...
#
	...
	Total <- as.matrix( NA, nStrat, 2 )
	females <- as.matrix( NA, nStrat, 2 )
	Young <- as.matrix( NA, nStrat, 2 )
	Yrlg_Males <- as.matrix( NA, nStrat, 2 )
	SubAd_Males <- as.matrix( NA, nStrat, 2 )
	Ad_Males <- as.matrix( NA, nStrat, 2 )
	Unclass <- as.matrix( NA, nStrat, 2 )
	...
	for( iStrat in strats ){
		...
		Total[iStrat,1] <- (data$Popn_SU[1]/data$Sample_SU[1])*sum( Tot.SU )			# total population size
		females.SU <- by( data$"1+_Females"/data$p, data$Standard_SU, sum, na.rm=T )
		females[iStrat,1] <- (data$Popn_SU[1]/data$Sample_SU[1])*sum( females.SU )
	#	females <- sum( data$"1+_Females"/data$p, na.rm=T ) # female pop size
		...
		females[iStrat,2] <- sqrt(t(c(-data$"1+_Females"/data$p^2))%*%vc.P%*%c(-data$"1+_Females"/data$p^2))
...
#
	...
	sdata <- getAerialSightData(input)

	if( input$critter == "Mule Deer" ){
		sdata2 <- sight.muleDeer( input, sdata )
	} else if ( input$critter == "Elk" && input$aircraft == "Hiller 12E" ){
		sdata2 <- sight.elkHillerSnow( input, sdata )
	} else if ( input$critter == "Elk" && input$aircraft == "Hiller 12E no snow" ){
		sdata2 <- sight.elkHillerNoSnow( input, sdata )
	} else if ( input$critter == "Elk" && input$aircraft == "Bell 47G" ){
		sdata2 <- sight.elkBell47G( input, sdata )
	} else if ( input$critter == "Elk" && input$aircraft == "Bell 206" ){
#		sdata2 <- sight.elkBell206( input, sdata )
	} else {
		cat( "Model unavailable for ", input$critter, ".\n"  )
		return(NULL)
	}
