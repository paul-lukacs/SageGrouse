...
########################################################################################
    ...
    model{
		...
		for(yr in 2:nyr){
      for(sex in 1:2){
				NN[yr,1,sex] <- round(((NN[yr-1,1,1] * FEC[yr-1,1]/2) +
                    (NN[yr-1,2,1] * FEC[yr-1,2]/2)) * PHI[yr-1,1,sex]) - HMN[yr,1,sex]
				NN[yr,2,sex] <- round((NN[yr-1,1,sex] + NN[yr-1,2,sex]) *
                                  PHI[yr-1,2,sex]) - HMN[yr,2,sex]
      }  #  sex
    }  #  y
